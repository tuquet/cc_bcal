...
def run_whisperx_job(job: dict, dry_run: bool, require_gpu: bool) -> dict:
    ...
    try:
        # Run the Docker command and show its output directly in the terminal.
...
def main():
    ...
    if args.episodes:
        for ep_name in args.episodes:
            # Find the project directory regardless of its video_type subfolder
            found = list(episodes_root.glob(f"*/{ep_name}"))
            if found:
                target_episode_dirs.append(found[0])
            else:
                print(f"Warning: Specified project directory not found: {ep_name}", file=sys.stderr)
    else:
        # Scan all subdirectories for project folders
        for type_dir in episodes_root.iterdir():
            if type_dir.is_dir():
                target_episode_dirs.extend([d for d in type_dir.iterdir() if d.is_dir()])
